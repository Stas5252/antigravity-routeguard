...
$InstalledBuildCommit = Join-Path $Root 'installed-build-commit.txt'
...
function Test-ManifestAt {
  param(
    [Parameter(Mandatory=$true)][string]$BaseDir,
    [switch]$Quiet
  )
  $manifest=Join-Path $BaseDir 'MANIFEST.sha256'
  if(!(Test-Path -LiteralPath $manifest)){
    if(-not $Quiet){ Say 'Package manifest not found; source/dev install detected. ZIP checksum verification is still recommended.' 'Yellow' }
    return $true
  }
...
function Install-Files {
  ...
  # Keep the user-facing launch/status/restore helpers in the persistent install
  # directory too. That way auto-update updates what the user actually runs.
  ...
  $buildFile=Join-Path $PSScriptRoot 'BUILD_COMMIT.txt'
  if(Test-Path -LiteralPath $buildFile){
    $commit=(Get-Content -LiteralPath $buildFile -Raw).Trim()
...
function Do-Update([switch]$Automatic) {
  ...
  try {
    ...
    $latestCommit=[string]$rel.target_commitish
    if($latestCommit -match '^[0-9a-fA-F]{40}
...
switch($Action){
  ...
  default{Menu}
    ...
    $rootFull=[IO.Path]::GetFullPath($BaseDir).TrimEnd('\\') + '\\'
    if(-not $full.StartsWith($rootFull,[StringComparison]::OrdinalIgnoreCase)){
      $fail += "Manifest path escapes package: $name"
      continue
    }
  ...
  if($listed -lt 8){ $fail += "Manifest is unexpectedly small ($listed entries)." }
  if($fail.Count -gt 0){
    throw "Package integrity check failed: $($fail -join '; ')"
  }
  if(-not $Quiet){ Say 'Package manifest: verified.' 'Green' }
  return $true
}

function Test-PackageManifest {
  return (Test-ManifestAt -BaseDir $PSScriptRoot)
}
...
switch($Action){
  ...
  default{Menu}
      ...
      Set-Content -LiteralPath $InstalledBuildCommit -Value $commit.ToLowerInvariant() -Encoding ASCII
    }
  }
}
...
switch($Action){
 ...
 -and (Test-Path $InstalledBuildCommit)){
      $installedCommit=(Get-Content $InstalledBuildCommit -Raw).Trim()
      if($installedCommit -eq $latestCommit.ToLowerInvariant()){
        Set-Content $InstalledReleaseHash $actual -Encoding ASCII
        Remove-Item $PendingUpdate -Force -ErrorAction SilentlyContinue
        if(-not $Automatic){ Say 'RouteGuard is already on the latest build.' 'Green' }
        return
      }
    }
    ...
    Set-Content $InstalledReleaseHash $actual -Encoding ASCII
    $bf=Join-Path $unpack 'BUILD_COMMIT.txt'
    if(Test-Path -LiteralPath $bf){
      $bc=(Get-Content $bf -Raw).Trim()
      if($bc -match '^[0-9a-fA-F]{40}
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $InstalledScript -Action Repair
    if(-not $Automatic){ Say 'RouteGuard updated, checksum verified, and repaired.' 'Green' }
  } catch {
    if($Automatic){
...
){ Set-Content $InstalledBuildCommit $bc.ToLowerInvariant() -Encoding ASCII }
    }
    Remove-Item $PendingUpdate -Force -ErrorAction SilentlyContinue
