...
function Do-Update([switch]$Automatic) {
  ...
  try {
    ...
    if($latestCommit -match '^[0-9a-fA-F]{40}$' -and (Test-Path $InstalledBuildCommit)){
    ...
    if(Test-Path -LiteralPath $bf){
      ...
      if($bc -match '^[0-9a-fA-F]{40}$'){ Set-Content $InstalledBuildCommit $bc.ToLowerInvariant() -Encoding ASCII }
    }
    Remove-Item $PendingUpdate -Force -ErrorAction SilentlyContinue
